`timescale 1ns / 1ps

module mem_wb_regs(
    input clk,
    input rst,
       input [31:0] alu_result_in,    
    input [31:0] mem_read_data_in, // Data loaded from RAM (for LW)
    input [31:0] pc_plus_4_in,      
    input [31:0] imm_val_in,        
      input [4:0]  write_reg_num_in, // The 'rd' destination address  
      input        reg_write_in,     // write to the register
    input        mem_to_reg_in,    ///writing memory data, 0 for ALU data
    input        jump_in,          // writing PC+4
    input        lui_operation_in, // writing the immediate directly

   
    
    output reg [31:0] alu_result_out,
    output reg [31:0] mem_read_data_out,
    output reg [31:0] pc_plus_4_out,
    output reg [31:0] imm_val_out,
    
    output reg [4:0]  write_reg_num_out,

    output reg        reg_write_out,
    output reg        mem_to_reg_out,
    output reg        jump_out,
    output reg        lui_operation_out
);

    always @(posedge clk) begin
        if (rst) begin
            alu_result_out    <= 0;
            mem_read_data_out <= 0;
            pc_plus_4_out     <= 0;
            imm_val_out       <= 0;
            write_reg_num_out <= 0;
            
            reg_write_out     <= 0;
            mem_to_reg_out    <= 0;
            jump_out          <= 0;
            lui_operation_out <= 0;
        end else begin
            alu_result_out    <= alu_result_in;
            mem_read_data_out <= mem_read_data_in;
            pc_plus_4_out     <= pc_plus_4_in;
            imm_val_out       <= imm_val_in;
            write_reg_num_out <= write_reg_num_in;
            
            reg_write_out     <= reg_write_in;
            mem_to_reg_out    <= mem_to_reg_in;
            jump_out          <= jump_in;
            lui_operation_out <= lui_operation_in;
        end
    end

endmodule